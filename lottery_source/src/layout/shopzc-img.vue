<template>
  <div class="load-box fh scroll">
    ...
    <!-- 照片规则提示层 -->
    <div class="load-prompt" v-show="showPrompt.hasClick">
      <div class="load-prompt-con">
        <img src="../public/default-img/zc-prompt.png" alt="prompt">
        <button class="btn-known" @click="handleKnownClick">知道了</button>
      </div>
    </div>
    <!-- img cropper -->
...
import zwCropper from '@/components/imgCropper'
...
export default {
  ...
  data: function() {
    return {
      ...
      // 照片规则提示层 参数
      showPrompt: {
        hasClick: false  // 是否点击过
      },
      isScroll: true, // 页面是否滚动
  ...
  methods: {
    ...
    handleShowloadPrompt () {
      // 是否点击过
      this.showPrompt.hasClick = true
      // 固定页面
      this.isScroll = false
    },
    // 照片规则提示层 点击事件
    handleKnownClick () {
      this.showPrompt.hasClick = false
      // 滚动页面
      this.isScroll = true
    }
  },
