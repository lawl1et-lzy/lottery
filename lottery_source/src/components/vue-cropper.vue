		...
		getCropData (cb) {
			...
			img.onload = () => {
				if (~~(this.cropW) !== 0) {
					...
					let expectWidth = trueWidth
					let expectHeight = trueHeight
					if (trueWidth > trueHeight && trueWidth > 800) {
							expectWidth = 800
							expectHeight = expectWidth * trueHeight / trueWidth
					} else if (trueHeight > trueWidth && trueHeight > 1200) {
							expectHeight = 1200
							expectWidth = expectHeight * trueWidth / trueHeight
					}
					let width = expectWidth
					let height = expectHeight
					let ctx = canvas.getContext('2d')
					ctx.save()
    			switch (rotate) {
    				case 0:
